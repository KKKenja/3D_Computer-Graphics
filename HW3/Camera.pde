public class Camera {
    ...
    void setPositionOrientation(Vector3 pos, float rotX, float rotY) {
        // rotX: pitch (X), rotY: yaw (Y)
        Matrix4 R = Matrix4.RotY(rotY).mult(Matrix4.RotX(rotX)); // yaw then pitch
        Matrix4 Rt = R.transposed();
        Matrix4 Tinv = Matrix4.Trans(pos.mult(-1));
        worldView = Rt.mult(Tinv);
        transform.position = pos;
        transform.rotation = new Vector3(rotX, rotY, 0);
    }
